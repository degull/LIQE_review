...
class LIQE(nn.Module):
    ...
    def forward(self, x):
        ...
        sel_step = x.size(1) // self.num_patch  # 이미지 패치 중 일부만 선택헤 처리
        sel = torch.arange(0, sel_step * self.num_patch, sel_step).long()  # 수정된 부분
        x = x[:, sel, ...]  # 여기서 차원을 선택해야 합니다.

        x = x.view(-1, 3, 224, 224)  # 수정된 부분

        image_features = self.model.encode_image(x)     # 이미지 임베딩 생성
        # self.model.encode_image(x) : CLIP 모델을 사용해 이미지 패치를 임베딩 공간으로 변환 -> 이후 텍스트 임베딩과의 비교에 사용됨

        # normalized features
        image_features = image_features / image_features.norm(dim=1, keepdim=True)  # 임베딩 정규화
