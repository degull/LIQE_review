...
for i, img_file in enumerate(image_files):
    ...
    quality_prediction = logits_quality.argmax(dim=1).float().mean()
    predicted_quality_scores.append(quality_prediction.cpu().item())

    # Scene prediction
    scene_prediction = logits_per_image.sum(3).mean(dim=1).argmax(dim=1).float().mean()
    predicted_scene_scores.append(scene_prediction.cpu().item())

    # Distortion prediction
    distortion_prediction = logits_per_image.sum(2).mean(dim=1).argmax(dim=1).float().mean()
