...
# Separate Task Templates
quality_texts = torch.cat([clip.tokenize(f"a photo with {q} quality") for q in qualitys]).to(device)
scene_texts = torch.cat([clip.tokenize(f"a photo of a {c}") for c in scenes]).to(device)
dist_texts = torch.cat([clip.tokenize(f"a photo with {d} artifacts") for d in dists_map]).to(device)

img1 = 'C:/Users/IIPL02/Desktop/LIQE/LIQE/data/6898804586.jpg'
img2 = 'C:/Users/IIPL02/Desktop/LIQE/LIQE/data/I02_01_03.png'
...
with torch.no_grad():
    logits_per_image1, _ = do_batch(I1_patches.unsqueeze(0), quality_texts)
    logits_per_image2, _ = do_batch(I2_patches.unsqueeze(0), quality_texts)

print('###Marginalization###')

logits_per_image1 = logits_per_image1.view(-1, len(qualitys))
quality_prediction1 = 1 * logits_per_image1[:, 0] + 2 * logits_per_image1[:, 1] + 3 * logits_per_image1[:, 2] + \
                4 * logits_per_image1[:, 3] + 5 * logits_per_image1[:, 4]

logits_per_image2 = logits_per_image2.view(-1, len(qualitys))
quality_prediction2 = 1 * logits_per_image2[:, 0] + 2 * logits_per_image2[:, 1] + 3 * logits_per_image2[:, 2] + \
                4 * logits_per_image2[:, 3] + 5 * logits_per_image2[:, 4]

print('Image #1 has a perceptual quality of {} as quantified by LIQE'.format(quality_prediction1.item()))
print('Image #2 has a perceptual quality of {} as quantified by LIQE'.format(quality_prediction2.item()))

# Scene and Distortion prediction
with torch.no_grad():
    logits_per_image1_scene, _ = do_batch(I1_patches.unsqueeze(0), scene_texts)
    logits_per_image1_dist, _ = do_batch(I1_patches.unsqueeze(0), dist_texts)
    
    logits_per_image2_scene, _ = do_batch(I2_patches.unsqueeze(0), scene_texts)
    logits_per_image2_dist, _ = do_batch(I2_patches.unsqueeze(0), dist_texts)

logits_per_image1_scene = logits_per_image1_scene.view(-1, len(scenes))
scene_index1 = logits_per_image1_scene.argmax(dim=1)
scene_prediction1 = scenes[scene_index1]

logits_per_image1_dist = logits_per_image1_dist.view(-1, len(dists_map))
distortion_index1 = logits_per_image1_dist.argmax(dim=1)
distortion_prediction1 = dists_map[distortion_index1]

logits_per_image2_scene = logits_per_image2_scene.view(-1, len(scenes))
scene_index2 = logits_per_image2_scene.argmax(dim=1)
scene_prediction2 = scenes[scene_index2]

logits_per_image2_dist = logits_per_image2_dist.view(-1, len(dists_map))
distortion_index2 = logits_per_image2_dist.argmax(dim=1)
distortion_prediction2 = dists_map[distortion_index2]

print('Image #1 is a photo of {} with {} artifacts'.format(scene_prediction1, distortion_prediction1))
print('Image #2 is a photo of {} with {} artifacts'.format(scene_prediction2, distortion_prediction2))
