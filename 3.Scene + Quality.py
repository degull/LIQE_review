...
def evaluate_and_print_results(model, train_loader, val_loader, test_loader, best_result, best_epoch, srcc_dict):
    ...
    srcc_avg = (srcc_quality_val + srcc_scene_val) / 2
    
    table = PrettyTable()
    table.field_names = ["Task Combination", "SRCC Quality", "SRCC Scene"]
    ...
    table.add_row(["Average", f"{srcc_avg:.3f}", f"{srcc_scene_test:.3f}"])
